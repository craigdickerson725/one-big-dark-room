...
class BandListingViewsTestCase(TestCase):
    ...
    def test_band_listing_list_view(self):
        # Assuming the name of the list view URL is 'index'
    ...
    def test_create_listing_view(self):
        ...
        self.assertTrue(
            BandListing.objects.filter(band_name='New Band').exists())

    def test_band_listing_detail_view(self):
        response = self.client.get(reverse('bandlisting_detail', kwargs={'slug': self.band_listing.slug}))  # noqa
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, 'band_listing/bandlisting_detail.html')
        self.assertContains(response, 'Test Band')

    def test_edit_listing_view(self):
        self.client.login(username='testuser', password='password')
        response = self.client.post(reverse('edit_listing', kwargs={'slug': self.band_listing.slug}),  # noqa
        {
    ...
    def test_delete_listing_view(self):
        ...
        response = self.client.post(
            reverse('delete_listing', kwargs={'slug': self.band_listing.slug}))
        # Should redirect after successful deletion
        self.assertEqual(response.status_code, 302)
        self.assertFalse(BandListing.objects.filter(
            pk=self.band_listing.pk).exists())

    def test_send_message_view(self):
        self.client.login(username='testuser', password='password')
        response = self.client.post(reverse('send_message', kwargs={'slug': self.band_listing.slug}),  # noqa
        {
            'message_body': 'Hello!',
        })
        # Should redirect after successful send
        self.assertEqual(response.status_code, 302)
    ...
    def test_message_detail_view(self):
        ...
        response = self.client.get(
            reverse('message_detail', kwargs={'pk': message.pk}))
        self.assertEqual(response.status_code, 200)
